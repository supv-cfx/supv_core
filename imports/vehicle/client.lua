...
function SetVehicleProperties(vehicle, props, restricted)
	if not DoesEntityExist(vehicle) then return nil end
    if restricted then
	    if NetworkGetEntityIsNetworked(vehicle) and NetworkGetEntityOwner(vehicle) ~= supv.oncache.playerid then return nil end
    end
...
function SpawnVehicle(vehicle, coords, cb)
    local vehicleCreated, id

    CreateThread(function()

        supv.stream.request(vehicle)
        vehicleCreated = CreateVehicle(vehicle, coords.x, coords.y, coords.z, coords.w, true, false)


        id = NetworkGetNetworkIdFromEntity(vehicleCreated)
        SetNetworkIdCanMigrate(id, true)
        SetEntityAsMissionEntity(vehicleCreated, true, false)

        SetVehicleHasBeenOwnedByPlayer(vehicleCreated, true)
        SetVehicleNeedsToBeHotwired(vehicleCreated, false)
        SetModelAsNoLongerNeeded(vehicle)
        SetVehRadioStation(vehicleCreated, 'OFF')
        RequestCollisionAtCoord(coords.xyz)

        while not HasCollisionLoadedAroundEntity(vehicleCreated) do
            Wait(0)
        end
...
function SpawnLocalVehicle(vehicle, coords, cb)
    ...
    CreateThread(function()
        ...
        supv.stream.request(vehicle)
        vehicleCreated = CreateVehicle(vehicle, coords.x, coords.y, coords.z, coords.w, false, false)

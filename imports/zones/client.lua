-- credit: ox_lib <https://github.com/overextended/ox_lib/blob/master/imports/zones/client.lua>
if lib then return lib.zones end
...
CreateThread(function()
    ...
    while true do
        ...
        for _, zone in pairs(Zones) do
            zone.distance = player:distance(zone.coords)
            local radius, contains = zone.radius

            if radius then
                contains = zone.distance < radius
            else
                contains = glm_polygon_contains(zone.polygon, player.coords, zone.thickness / 4)
            end

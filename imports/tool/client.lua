--[[
    :fr:
    - C'est tool provient dans ma version sur la branche béta.
    - Ils seront refait ultérieurement en mieux.

    :en:
    - This tool comes from my version on the beta branch.
    - They will be redone later in better.
--]]
...
local function RayCastCamera(distance)
	...
	local _, b, c, _, e = GetShapeTestResult(StartShapeTestRay(cameraCoord.x, cameraCoord.y, cameraCoord.z, destination.x, destination.y, destination.z, -1, supv.cache.ped, 0))
	return b, c, e
end

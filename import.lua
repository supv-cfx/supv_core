...
local service <const> = (IsDuplicityVersion() and 'server') or (not IsDuplicityVersion() and 'client') or ('shared')
...
local function load_module(self, index)
    ...
    if err then
        error(("Erreur pendant le chargement du module\n- Provenant de : %s\n- Modules : %s\n- Service : %s\n - Erreur : %s"):format(dir, index, service, err), 3)
    end
...
supv = setmetatable({name = supv_core, service = service, oncache = {}, config = {client = {}, server = {}, shared = {}}},{ __index = call_module, __call = call_module })
...
elseif service == 'server' then
    RegisterNetEvent('supv_core:insert:config-server', function(cfg)
        for k,v in pairs(cfg) do
            supv.config.server[k] = v
        end 
        return supv.config.server   
    end)
elseif service == 'shared' then
    RegisterNetEvent('supv_core:insert:config-shared', function(cfg)
        for k,v in pairs(cfg) do
            supv.config.shared[k] = v
        end
        return supv.config.shared
    end)
end